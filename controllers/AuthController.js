...
static async getConnect(req, res) {
    try {
      ...
      const token = req.headers['x-token'] || uuidv4();
      console.log('Token being set in Redis:', token);

      await redisClient.set(`auth_${token}`, user._id.toString(), 'EX', 86400)
        .then(() => console.log('Token successfully set in Redis'))
        .catch((err) => console.error('Error setting token in Redis:', err));
